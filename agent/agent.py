...
def _route(question: str, context: str) -> dict:
    ...
    return llm.complete_json(
        ...
        "column (cast to int) — NEVER an aggregate/count row. If the question STATES the interim counts "
        "directly ('40 patients in with 12 responses') instead of naming a warehouse table, synthesize "
        "the per-subject rows with range(): `SELECT 1 AS responded FROM range(12) UNION ALL SELECT 0 AS "
        "responded FROM range(28)` (successes, then n_observed - successes failures). Also extract "
        "`n_planned` (the FULL planned enrolment, which is larger than the rows returned), `tv`, `lrv`, "
        ...
        "on the drug vs 10/38 on control') set `framing`='two_arm', and ALWAYS set ALL THREE of "
        "`outcome`=the binary response column, `group`=the arm column, and `control`=the control arm's "
        "value. Express `tv`/`lrv` as RISK DIFFERENCES treatment-minus-control (e.g. a 15-point benefit "
        "hoped for -> tv 0.15; any benefit is the floor -> lrv 0). analytic_sql returns one row per "
        "subject with the arm column + a binary outcome column; when counts are stated directly, "
        "synthesize each arm's rows with range() (e.g. `SELECT 'treatment' AS arm, 1 AS responded FROM "
        "range(18) UNION ALL ...` -> then \"outcome\":\"responded\", \"group\":\"arm\", "
        "\"control\":\"control\"). n_planned is the TOTAL planned enrolment across both arms.\n"
